...
#------------------------------------------------------------------------------------------
  ...
  #     it '会員情報の名前が正しい' do
  #       profile_name = all(:css, ".customer-profile-test")[0].native.text
  #       expect(profile_name).to match(/#{customer2.name}/)
  #     end
  #     it '自己紹介がきちんと表示されているか' do
  #       profile_name = all(:css, ".customer-introduction-test")[0].native.text
  #       expect(profile_name).to match(/#{customer2.introduction}/)
  #     end
  #     it 'フォロー数リンクが表示される' do
  #       link = all(:css, ".follow-index-btn-test")[0].find_all("a")[0].native.text
  #       expect(link).to match(/フォロー数: #{customer2.followings.count}/)
  #     end
  #     it 'フォロー数リンクの内容が正しい' do
  #       all(:css, ".follow-index-btn-test")[0].find_all("a")[0].click
  #       expect(current_path).to eq (customer_followings_path(customer2))
  #     end
  #     it 'フォロー数が増える' do
  #       expect { create(:relationship,follower_id: customer2.id, followed_id: customer.id) }.to change{customer2.followings.count}.by(1)
  #       visit customer_path(customer2)
  #       link = all(:css, ".follow-index-btn-test")[0].find_all("a")[0].native.text
  #       expect(link).to match(/フォロー数: 1/)
  #     end
  #     it 'フォロワー数リンクが表示される' do
  #       link = all(:css, ".follow-index-btn-test")[0].find_all("a")[1].native.text
  #       expect(link).to match(/フォロワー数: #{customer2.followers.count}/)
  #     end
  #     it 'フォロワー数リンクの内容が正しい' do
  #       all(:css, ".follow-index-btn-test")[0].find_all("a")[1].click
  #       expect(current_path).to eq (customer_followers_path(customer2))
  #     end
  #     it 'フォロワー数が増える' do
  #       expect { create(:relationship,follower_id: customer.id, followed_id: customer2.id) }.to change{customer2.followers.count}.by(1)
  #       visit customer_path(customer2)
  #       link = all(:css, ".follow-index-btn-test")[0].find_all("a")[1].native.text
  #       expect(link).to match(/フォロワー数: 1/)
  #     end
  ...
  #       expect(customer.followings.count).to eq 1
  #     end
  ...
  #       it "フォローが外せる" do
  #       all(:css, ".follow-test")[0].find_all("a")[0].click
  #       expect(customer.followings.count).to eq 0
  #       end
  #     end

  #     context "フォローされている状態" do
  #       let!(:relationship) { create(:relationship,follower_id: customer2.id, followed_id: customer.id) }
  ...
  #     it 'お気に入りグループリンクの内容が正しい' do
  #       all(:css, ".group-btn-test")[0].find_all("a")[0].click
  #       expect(current_path).to eq (customer_group_favorites_path(customer2))
  #     end
  #     it '作成したグループリンクが表示される' do
  ...
  #     it '作成したグループリンクの内容が正しい' do
  #       all(:css, ".group-btn-test")[0].find_all("a")[1].click
  #       expect(current_path).to eq (customer_groups_path(customer2))
  #     end
  #     it 'いいね一覧リンクが表示される' do
  ...
  #     it 'いいね一覧リンクの内容が正しい' do
  #       all(:css, ".index-btn-test")[0].find_all("a")[0].click
  #       expect(current_path).to eq (customer_post_favorites_path(customer2))
  #     end
  #     it 'コメント一覧リンクが表示される' do
  ...
  #     it 'コメント一覧へリンクの内容が正しい' do
  #       all(:css, ".index-btn-test")[0].find_all("a")[1].click
  #       expect(current_path).to eq (customer_post_comments_path(customer2))
  #     end
  #   end
  ...
  #     context '投稿が存在しないとき' do
  #       it '見出しの確認' do
  #         expect(all(:css, ".midashi")[1].native.text).to match(/#{customer2.name}の投稿一覧/)
  #       end
  #       it '投稿が存在しない場合の表示' do
  #         expect(page).to have_content(/#{customer2.name}の投稿が存在しません/)
  #       end
  #     end

  #     context '投稿が存在する場合' do
  #       let!(:posts) { create_list(:post,21,customer_id: customer2.id) }
  #       let!(:tag_post) { create(:tag_post) }
  #       let!(:post_tag_post) { create(:post_tag_post,post_id: posts.last.id, tag_post_id: tag_post.id) }
  #       before do
  #         visit customer_path(customer2)
  ...
  #         expect(profile_image).to match(/#{customer2.name}/)
  #       end
  #       it '会員リンクの内容が正しい' do
  #         all(:css, ".customer-profile-test")[1].click
  #         expect(current_path).to eq (customer_path(customer2))
